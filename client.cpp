...
int DoClientWork(char* name, char* port){
	...
	char send_data_with_name[BUFSIZE + MAXNAME]; //toto: need to gracefully exit upon EOF
	while(scanf("%s", send_data) != EOF){
		strcpy(send_data_with_name, name);
		strcat(send_data_with_name, ":: ");
		strcat(send_data_with_name, send_data);
		if (sendto(g_fdclient, send_data_with_name, strlen(send_data_with_name), 0, (struct sockaddr *)&g_remaddrclient, slen)==-1) {
			perror("sendto");
			exit(1);
		}
	}
